...
def generate(slide_num, model=None,
             spk1_name=None, spk1_voice=None,
             spk2_name=None, spk2_voice=None,
             preamble=None,
             voiceover_file=None,
             output_folder=None):
    model = model or env_or_default("TTS_MODEL", "gemini-2.5-flash-preview-tts")
    spk1_name = spk1_name or env_or_default("TTS_SPK1_NAME", "Amanda")
    spk1_voice = spk1_voice or env_or_default("TTS_SPK1_VOICE", "Zephyr")
    spk2_name = spk2_name or env_or_default("TTS_SPK2_NAME", "Jack")
    spk2_voice = spk2_voice or env_or_default("TTS_SPK2_VOICE", "Fenrir")
    preamble = preamble or env_or_default("TTS_PREAMBLE", "Read aloud in a warm, welcoming tone with British (Brixton) accent:")
    voiceover_file = voiceover_file or env_or_default("TTS_VOICEOVER_FILE", "voiceover_3.txt")
    output_folder = output_folder or env_or_default("TTS_OUTPUT_FOLDER", "audio")
    ...
    speaker_style_guide = (
        ...
        f"- {spk1_name}: adult female lecturer voice, clear and steady.\n"
        f"- {spk2_name}: adult male student voice, distinctly masculine timbre.\n"
        "- Keep both voices consistent across the entire script.\n"
        "- Never swap or blend speaker identities.\n"
        "- Preserve the original dialogue text and speaker turns exactly."
    )
...
if __name__ == "__main__":
    ...
    spk1_name = sys.argv[3] if len(sys.argv) > 3 else None
    spk1_voice = sys.argv[4] if len(sys.argv) > 4 else None
    spk2_name = sys.argv[5] if len(sys.argv) > 5 else None
    spk2_voice = sys.argv[6] if len(sys.argv) > 6 else None
    preamble = sys.argv[7] if len(sys.argv) > 7 else None
    voiceover_file = sys.argv[8] if len(sys.argv) > 8 else None
    output_folder = sys.argv[9] if len(sys.argv) > 9 else None
