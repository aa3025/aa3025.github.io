...
out_base = Path('/Volumes/Elements-aa3025/GitLab/aa3025.github.io/bfmg')
...
def build_page_html(current_item, is_post=False):
    root_rel = "../" if is_post else "./"
    media_rel = root_rel + "media/"
    ...
    for p in pages:
        ...
        p_filename = "index.html" if is_p_home else f"{p['slug']}.html"
        p_link = root_rel + p_filename
    ...
    if current_item['slug'] == 'posts':
        ...
        for p in posts:
            post_url = root_rel + f"posts/{p['slug']}.html"
            all_posts_grid += f'''
...
<body class="antialiased text-slate-100 min-h-screen flex flex-col justify-between">
    ...
    <header class="sticky top-0 z-50 px-4 py-4 backdrop-blur-md bg-slate-950/40 border-b border-white/10">
        <div class="max-w-7xl mx-auto flex flex-wrap items-center justify-between gap-4">
            <a href="{root_rel}index.html" class="flex items-center gap-3 group text-decoration-none">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-8">
            ...
            <aside class="lg:col-span-4 space-y-6">
                ...
                <div class="glass-panel p-6">
                    ...
                    <div class="space-y-2">
                        {"".join([f'<a href="{root_rel}{"index.html" if page["slug"] == "home" or page["title"].lower() in ["home", "about bfmg", "about"] else page["slug"] + ".html"}" class="flex items-center justify-between p-2.5 rounded-xl bg-white/5 border border-white/10 text-sm text-slate-200 hover:bg-white/10 hover:text-white transition-all"><span class="flex items-center gap-2"><i data-lucide="file-text" class="w-4 h-4 text-blue-300"></i> {page["title"]}</span><i data-lucide="chevron-right" class="w-4 h-4 opacity-50"></i></a>' for page in pages])}
                    </div>
                </div>
...
for p in pages:
    file_name = "index.html" if p['slug'] == 'home' or p['title'].lower() in ['home', 'about bfmg', 'about'] else f"{p['slug']}.html"
    out_path = out_base / file_name
    print(f"Writing Page: {out_path}")
    with open(out_path, 'w', encoding='utf-8') as f:
        f.write(build_page_html(p, is_post=False))

# Make sure index.html exists
if not (out_base / 'index.html').exists():
    first_page = pages[0] if pages else {{
        'title': 'British Federation of Mathematical Games',
        'slug': 'index',
        'formatted_date': '',
        'content': '<h2>Welcome to BFMG</h2><p>Explore the posts and results in the sidebar.</p>',
        'type': 'page'
    }}
    with open(out_base / 'index.html', 'w', encoding='utf-8') as f:
        f.write(build_page_html(first_page, is_post=False))
...
for p in posts:
    ...
    with open(out_path, 'w', encoding='utf-8') as f:
        f.write(build_page_html(p, is_post=True))
